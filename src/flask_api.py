...
# ✅ React build folder path (because flask_api.py is inside src/)
...
# ✅ Dashboard Home + React fallback
@app.route("/", defaults={"path": ""})
@app.route("/<path:path>")
def serve_dashboard(path):
    file_path = os.path.join(app.static_folder, path)

    # serve actual files (JS/CSS/assets)
...
def health():
    ...
    return jsonify(
        {
            "status": "healthy"
            if all([model_loaded, scaler_loaded, encoders_loaded])
            else "degraded",
            "components": {
...
def predict():
    try:
        ...
        data = request.json
        if not data:
            return jsonify({"success": False, "error": "No data provided. Please send claim data as JSON."}), 400
...
def batch_predict():
    try:
        ...
        if not data or "claims" not in data:
            return jsonify({"success": False, "error": 'Expected format: {"claims": [...]}'}), 400

        claims = data["claims"]

        if not isinstance(claims, list) or len(claims) == 0:
            return jsonify({"success": False, "error": "Claims must be a non-empty list"}), 400

        if len(claims) > 100:
            return jsonify({"success": False, "error": "Maximum 100 claims per batch request"}), 400
